...
HtmlTerrain.prototype.applyTerrainTransform = function(afuckingname) {
	var geometry = afuckingname(self.terrain.geometry);
	...
	console.log("Transforming Terrain...");
	return geometry;
	
};

HtmlTerrain.prototype.terrainRuss = function(geometry) {
	...
	console.log(geometry.vertices[i]);
		n = noise.noise(geometry.vertices[i].position.x / self.terrain.resolution.x / 10, geometry.vertices[i].position.y / self.terrain.resolution.y / 10); // Position of X and Y Vertice Divided Twice by resolution and other number.
		n -= 0.5;
		geometry.vertices[i].position.z = n * 80; // Z-axis selected so it builds along z-axis outward
	}
...
HtmlTerrain.prototype.animate = function() {
				requestAnimationFrame( self.animate );
				self.loop();
			}

HtmlTerrain.prototype.loop = function() {
	console.log("Loop RAN!");
	self.applyTerrainTransform(self.terrainRandomHeightMap);
	}
